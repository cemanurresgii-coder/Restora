...
import { CalendarCheck, Clock, Users, MessageSquare, Inbox, CheckCircle2, XCircle, Hourglass, MapPin } from 'lucide-react'
import { subscribeUserReservations, updateReservationStatus, RESERVATION_STATUS } from '../../services/reservationService'
...
/** Statuses the customer can still cancel */
...
export default function MyReservationsContent() {
  ...
  const handleCancel = async (id) => {
    setCancellingId(id)
    try {
      await updateReservationStatus(id, RESERVATION_STATUS.CANCELLED)
              ...
              <div key={r.id} className="bg-white rounded-2xl border border-neutral-200 p-5 shadow-sm">
                ...
                {canCancel && (
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-medium text-red-500 hover:text-red-600 border border-red-200 hover:border-red-300 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition disabled:opacity-50"
                  >
                    {isCancelling
                      ? <><span className="animate-spin inline-block h-3 w-3 border border-red-400 border-t-transparent rounded-full" /> Cancelling…</>
